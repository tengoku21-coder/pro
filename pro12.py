...
def calc_station(station_n_chargers, station_daily_kwh, capex_per_unit,
    ...
    total_capex = capex_per_unit * station_n_chargers
    ...
    for m in range(station_months):
        ...
        records.append({
            ...
            "전력량요금_단가": s_rate * kepco_mult[m],
            "매출_합계": revenue,
            "한전_기본": kepco_base_cost,
            "한전_전력량": kepco_var_cost,
            "한전_합계": kepco_base_cost + kepco_var_cost,
            "통신비": comm_cost,
            "A/S비": as_cost,
            "영업비_합계": opex,
            "EBITDA": ebitda,
    ...
    return {
        ...
        "payback": payback,
        "total_revenue": df["매출_합계"].sum(),
        "total_opex": df["영업비_합계"].sum(),
...
st.dataframe(
    station_summary.style.format(fmt_dict).applymap(
...
for sc_name in SCENARIO_OPTIONS:
    ...
    if sc_dfs:
        ...
        for col in ["매출_합계", "영업비_합계", "당기순이익", "EBITDA", "월현금흐름"]:
            padded = []
            for d in sc_dfs:
                vals = d[col].values
                if len(vals) < sc_max:
...
with tab2:
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=all_combined["연월"], y=all_combined["매출_합계"],
                             mode="lines", name="매출", fill="tozeroy", line=dict(color="#2ECC71")))
    fig.add_trace(go.Scatter(x=all_combined["연월"], y=all_combined["영업비_합계"],
...
with st.expander("🔍 개별 충전소 상세 월별 손익"):
    ...
    key_cols = [
        "월차", "연월", "충전량_kWh", "충전요금_단가", "전력량요금_단가", "매출_합계",
        "한전_합계", "통신비", "A/S비", "영업비_합계",
        "EBITDA", "감가상각", "금융비용", "당기순이익", "월현금흐름", "누적현금흐름",
    ]
    st.dataframe(
...
monthly_rev_model = all_combined["매출_합계"].iloc[0] if len(all_combined) > 0 else 0
