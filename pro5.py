...
- 시나리오 입력 전용
- 월별 사용량 변화(증가율 적용)
- 시간대/계시별(TOU) 전력요금: 경/중/최 요금 & 사용비중
설치:
...
class Inputs:
    ...
    # TOU: 경/중/최 요금(원/kWh) & 사용비중(% → 0~1로 저장)
    tou_rate_off:float
    tou_rate_mid:float
    tou_rate_peak:float
    tou_share_off:float
    tou_share_mid:float
    tou_share_peak:float
...
def build_cashflows(inp:Inputs)->Tuple[pd.DataFrame,List[float],float,Optional[float],Optional[int],Optional[float],float]:
    ...
    for m in range(1, months+1):
        ...
        energy_cost_per_unit_off  = kwh_off  * inp.tou_rate_off
        energy_cost_per_unit_mid  = kwh_mid  * inp.tou_rate_mid
        energy_cost_per_unit_peak = kwh_peak * inp.tou_rate_peak
        energy_cost_per_unit = energy_cost_per_unit_off + energy_cost_per_unit_mid + energy_cost_per_unit_peak

        revenue_per_unit = kwh_unit * sell_price

        # 전체(사이트)
        revenue_total = revenue_per_unit * inp.total_units
        energy_off_total  = energy_cost_per_unit_off  * inp.total_units
        energy_mid_total  = energy_cost_per_unit_mid  * inp.total_units
        energy_peak_total = energy_cost_per_unit_peak * inp.total_units
...
st.caption("· ‘한전불입 대수 + 모자분리 대수 = 총 대수’ 자동 · 월별 사용량 증가율 · TOU(경/중/최) 전력요금 적용")
...
for i,tab in enumerate(tabs,start=1):
    with tab:
        ...
        # TOU 요금 & 비중
        with st.expander(f"[S{i}] 전력요금(TOU) · 경/중/최 요금 & 사용비중"):
            col1,col2,col3 = st.columns(3)
            with col1:
                tou_rate_off=st.number_input(f"경부하 요금(원/kWh) [S{i}]",0.0,5000.0,90.0,5.0,key=f"tr_off{i}")
                tou_share_off=st.number_input(f"경부하 비중(%) [S{i}]",0.0,100.0,40.0,1.0,key=f"ts_off{i}")/100.0
            with col2:
                tou_rate_mid=st.number_input(f"중간부하 요금(원/kWh) [S{i}]",0.0,5000.0,120.0,5.0,key=f"tr_mid{i}")
                tou_share_mid=st.number_input(f"중간부하 비중(%) [S{i}]",0.0,100.0,40.0,1.0,key=f"ts_mid{i}")/100.0
            with col3:
                tou_rate_peak=st.number_input(f"최대부하 요금(원/kWh) [S{i}]",0.0,5000.0,180.0,5.0,key=f"tr_peak{i}")
                tou_share_peak=st.number_input(f"최대부하 비중(%) [S{i}]",0.0,100.0,20.0,1.0,key=f"ts_peak{i}")/100.0

            # 비중 정규화 안내
            ssum = tou_share_off + tou_share_mid + tou_share_peak
            if abs(ssum-1.0) > 1e-6:
                st.caption(f"※ 현재 비중 합계: {ssum*100:.1f}% → 계산 시 자동 정규화됩니다.")
        ...
        inp=Inputs(
            ...
            tou_rate_mid=tou_rate_mid,
            tou_rate_peak=tou_rate_peak,
            tou_share_off=tou_share_off,
            tou_share_mid=tou_share_mid,
            tou_share_peak=tou_share_peak,
        ...
        st.dataframe(df[["월","사용전력(kWh/대)"]].head(24),use_container_width=True,height=240)
...
for k,v in scenario_results.items():
    ...
    rows.append({
        ...
        "경/중/최 요금": f"{int(inp.tou_rate_off)}/{int(inp.tou_rate_mid)}/{int(inp.tou_rate_peak)}"
    })
...
st.download_button("엑셀 다운로드 (요약+시나리오별 상세)",
                   ...
                   file_name="charger_scenarios_TOU.xlsx",
